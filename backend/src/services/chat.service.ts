...
export default class ChatService {
  ...
  async notifyRoomMembership(roomId: string, userId: string, action: 'joined' | 'left') {
    ...
    this.io.to(roomId).emit('membershipChange', { roomId, userId, action });
  ...
  setupSocketHandlers() {
    this.io.on('connection', (socket) => {
      // Validate user exists in socket data
      if (!socket.data.user) {
        socket.disconnect();
        return;
      }
  
      // Join course room with validation
      ...
      // Similar validation for cohort room joins
